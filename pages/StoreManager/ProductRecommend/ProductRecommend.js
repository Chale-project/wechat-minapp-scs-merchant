// pages/StoreManager/ProductRecommend/ProductRecommend.js
...
Page({
  ...
  getproductlistdata:function () {
    ...
    // }
    let para = { "currentPage": this.data.pageNumber, "pageSize": 10, "where": { "isMarketable": "put", "shopId": wx.getStorageSync("chooseshopid"), "categoryId": this.data.chooseid, "title": this.data.searchtitle, "isRecommend":"noRecommend" } }
  ...
  storeaddproducetap: function (e) {
    ...
    if(idx==0){
      ...
      //查看推荐商品
      wx.navigateTo({
        url: '/pages/StoreManager/RecommendList/RecommendList',
      })
    }
  },
  //设置推荐接口
  setRecommenData:function () {
    // newRecommensetUrl
    let url = rootUrls.newRecommensetUrl + '/' + this.data.selids + '/' + 'recommend'
    console.log('推荐url',url)
    urlApi.getInfo(url, {}, "PUT")
      .then(res => {
        ...
        wx.showToast({
          icon: 'none',
          title: '已设置' + res.number ? res.number:0+ '个商品为推荐商品',
        })
      })
  },
  //选中商品
